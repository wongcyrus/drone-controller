...
class FormationManager:
    ...
    def move_to_formation(self, speed: int = 30, timeout: float = 60.0) -> bool:
        ...
        for iteration in range(max_iterations):
            ...
            # Prepare parallel movement commands
            movement_commands = []
            for drone_id, target_pos in self.formation_positions.items():
                ...
                if distance > 30:  # 30cm tolerance
                    ...
                    movement_commands.append((drone, int(dx), int(dy), int(dz), speed))

            # Execute all movements in parallel
            if movement_commands:
                futures = {}
                for drone, dx, dy, dz, spd in movement_commands:
                    future = self._executor.submit(drone.move, dx, dy, dz, spd)
                    futures[future] = drone.drone_id
                ...
                for future in as_completed(futures, timeout=30.0):
                    ...
                    try:
                        ...
                        self.logger.error(f"Movement failed for drone {drone_id}: {e}")
    ...
    def move_formation(self, dx: int, dy: int, dz: int, speed: int = 30) -> bool:
        ...
        # Move all drones simultaneously using parallel execution
        futures = {}
        for drone_id, drone in self.drones.items():
            if drone._is_flying:
                future = self._executor.submit(drone.move, dx, dy, dz, speed)
        ...
        success_rate = successful_moves / len(self.drones) if self.drones else 1.0
    ...
    def emergency_spread(self, spread_distance: float = 200) -> bool:
        ...
        # Execute emergency spread movements in parallel
        futures = {}
        for i, drone_id in enumerate(drone_ids):
            angle = math.radians(i * angle_step)
            ...
            # Calculate movement for this drone
            drone = self.drones[drone_id]
            current_pos = drone._current_position
            ...
            future = self._executor.submit(drone.move, dx, dy, 0, self.max_speed)
            futures[future] = drone_id
        ...
        for future in as_completed(futures, timeout=30.0):
            ...
            try:
                ...
                self.logger.error(f"Emergency spread failed for drone {drone_id}: {e}")

        self.logger.info(f"Emergency spread executed for {successful_moves}/{num_drones} drones")
        return successful_moves >= num_drones * 0.8  # 80% success rate

    def cleanup(self):
        """Cleanup resources when formation manager is no longer needed."""
        self.logger.info("Cleaning up FormationManager resources")
